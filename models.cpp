...
List b_rectangular(NumericVector s, double theta, NumericVector m) {
  ...
  for (int i = 0; i < S; i++) {
    b(i) = (errorf((theta+m(i))/(M_SQRT2*s(i))) + errorf((theta-m(i)) /
      (M_SQRT2*s(i))))/2.0;
    g(i) = (exp(-(m(i)+theta)*(m(i)+theta)/(2.0*s(i)*s(i))) -
      exp(-(m(i)-theta)*(m(i)-theta)/(2.0*s(i)*s(i)))) * M_1_SQRT_2PI/s(i);
  }
...
// [[Rcpp::export]]
List pairwise(double time, NumericVector state, List pars) {
  int S = pars["S"], i, j;
  ...
  for (i = 0; i < S - 1; i++) {
    for (j = i + 1; j < S; j++) {
      ...
      beta(i,j) = -alpha(i,j)*2.0*dm / (sv + w2);
      alpha(j,i) = alpha(i,j);
      beta(j,i) = -beta(i,j);
    }
  }
  for (i = 0; i < S; i++) {
    ...
    dvdt(i+S) = h2(i)*var(i)*(g(i) - beta_n) * smoothstep(n(i)*1.0e5);
  }
...
List evoHOI(double time, NumericVector state, List pars) {
  int S = pars["S"], i, j, k;
  double w = pars["w"], w2 = w*w, kappa = pars["kappa"], sv, dm, dm_ij, dm_jk, dm_ki,
    num, denom, epsilon_ijk, gamma_ijk, numgamma;
  NumericVector dvdt(2*S), n = state[Range(0, S - 1)], m = state[Range(S, 2*S - 1)];
  NumericVector sig = pars["sigma"], h2 = pars["h2"], var(S);
  NumericMatrix alpha(S,S), beta(S,S);
  List bg = b_rectangular(sig, pars["theta"], m);
  ...
  for (i = 0; i < S; i++) {
    ...
    for (j = 0; j < S; j++) {
      ...
      for (k = 0; k < S; k++) {
        ...
        epsilon_ijk = 2.0*w2*exp(-num/denom) / sqrt(M_SQRT_PI*w*denom);
        numgamma = 16.0*((m(i) - m(k))*var(j) + (m(i) - m(j))*var(k)) +
          4.0*w2*(2.0*m(i) - m(j) - m(k));
        gamma_ijk = -epsilon_ijk*numgamma/denom;
  ...
  for (i = 0; i < S; i++) {
    dvdt(i) = n(i)*(b(i)-alpha_n(i)-kappa*epsilon_nn(i))*smoothstep(n(i)*1.0e6);
    dvdt(i+S) = h2(i)*var(i)*(g(i)-beta_n(i)-kappa*gamma_nn(i))*smoothstep(n(i)*1.0e5);
  }
  return(List::create(dvdt));
}
...
List hierHOI(double time, NumericVector state, List pars) {
  ...
  for (i = 0; i < S; i++) {
    ...
    for (j = 0; j < S; j++) {
      ...
      beta_n(i) = beta_n(i) + beta(i,j)*n(j);
      for (k = 0; k < S; k++) {
        svH = 2.0*(var(i) + var(j) + var(k)) + W2;
        ddm = z0 + dm - m(k);
        epsilon_ijk = kappa * sigmoid(ddm / sqrt(svH));
        gamma_ijk = var(i) * exp(-ddm*ddm / svH) / sqrt(svH*M_PI);
        epsilon_nn(i) = epsilon_nn(i) + epsilon_ijk*n(j)*n(k);
        gamma_nn(i) = gamma_nn(i) + gamma_ijk*n(j)*n(k);
      }
    }
  ...
  for (i = 0; i < S; i++) {
    dvdt(i) = n(i)*(b(i)-alpha_n(i)-kappa*epsilon_nn(i))*smoothstep(n(i)*1.0e6);
    dvdt(i+S) = h2(i)*(g(i)-beta_n(i)-kappa*gamma_nn(i))*smoothstep(n(i)*1.0e5);
  }
  return(List::create(dvdt));
}
...
// [[Rcpp::export]]
List evoHOI_nopairwise(double time, NumericVector state, List pars) {
  int S = pars["S"], i, j, k;
  double w = pars["w"], w2 = w*w, kappa = pars["kappa"], dm_ij, dm_jk, dm_ki,
    num, denom, epsilon_ijk, gamma_ijk, numgamma;
  NumericVector dvdt(2*S), n = state[Range(0, S - 1)], m = state[Range(S, 2*S - 1)];
  NumericVector sig = pars["sigma"], h2 = pars["h2"], var(S);
  List bg = b_rectangular(sig, pars["theta"], m);
  NumericVector b = bg[0], g = bg[1];
  NumericVector epsilon_nn(S), gamma_nn(S);
  for (i = 0; i < S; i++) var(i) = sig(i)*sig(i);
  for (i = 0; i < S; i++) {
    epsilon_nn(i) = 0.0;
    gamma_nn(i) = 0.0;
  ...
  for (i = 0; i < S; i++) {
    dvdt(i) = n(i)*(b(i)-kappa*epsilon_nn(i))*smoothstep(n(i)*1.0e6);
    dvdt(i+S) = h2(i)*var(i)*(g(i)-kappa*gamma_nn(i))*smoothstep(n(i)*1.0e5);
  }
  return(List::create(dvdt));
}
...
// [[Rcpp::export]]
List randHOI(double time, NumericVector state, List pars) {
  int S = pars["S"], i, j, k;
  double w = pars["w"], w2 = w*w, dm, sv;
  NumericVector dvdt(2*S), n = state[Range(0, S - 1)], m = state[Range(S, 2*S - 1)];
  NumericVector sig = pars["sigma"], h2 = pars["h2"], var(S);
  NumericMatrix alpha(S,S), beta(S,S);
  List bg = b_rectangular(sig, pars["theta"], m);
  NumericVector b = bg[0], g = bg[1];
  NumericVector alpha_n(S), beta_n(S), epsilon_nn(S);
  arma::cube eps = pars["eps"];
  for (i = 0; i < S; i++) var(i) = sig(i)*sig(i);
  for (i = 0; i < S; i++) {
    alpha_n(i) = 0.0;
    beta_n(i) = 0.0;
    ...
    for (j = 0; j < S; j++) {
      ...
      for (k = 0; k < S; k++) {
        sv = sv + 2.0*var(k);
        dm = dm - m(k);
        epsilon_nn(i) = epsilon_nn(i) + eps.at(i,j,k)*n(j)*n(k);
      }
    }
  }
  for (i = 0; i < S; i++) {
    dvdt(i) = n(i)*(b(i) - alpha_n(i) - epsilon_nn(i)) * smoothstep(n(i)*1.0e6);
    dvdt(i+S) = h2(i)*var(i)*(g(i) - beta_n(i)) * smoothstep(n(i)*1.0e5);
  }
  return(List::create(dvdt));
}
